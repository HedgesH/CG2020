...
std::vector<float> interpolatefi(float start, float end, int size){
    float step_length = float((end-start))/(size-1);
  
    std::vector<float> result(size);
    for (size_t i = 0; i < size; i++){result[i] = start + (step_length*i);}
    return result;
};
...
std::vector<CanvasPoint> interpolateTextureAndCanvasPoint(CanvasPoint start, CanvasPoint end, int size){
    float step_lengthx = (end.x-start.x)/(size-1);
    float step_lengthy = (end.y-start.y)/(size-1);
    float step_lengthz = (end.depth-start.depth)/(size-1);
    ...
    for (size_t i = 0; i < size; i++){
        result[i] = CanvasPoint(roundl(start.x + (step_lengthx*i)), roundl(start.y + (step_lengthy*i)), start.depth + (step_lengthz*i));
...
void new_depth(CanvasPoint bottom, CanvasPoint top, CanvasPoint old_middle, CanvasPoint &middle){
    if(bottom.depth == top.depth){middle.depth = bottom.depth;}
    if(bottom.y != top.y){
        float m = (bottom.y - top.y) / (bottom.depth - top.depth);
        float c = top.y - (m*top.depth);
        middle.depth = (middle.y-c)/m;
    }
    else if(bottom.x != top.x){
        float m = (bottom.x - top.x) / (bottom.depth - top.depth);
        float c = top.x - (m*top.depth);
        middle.depth = (middle.x-c)/m;
    }
    else{
        middle.depth = old_middle.depth;
    }
}
...
std::vector<CanvasPoint> fill_triangle_points(CanvasPoint p, CanvasPoint lm, CanvasPoint rm){
    ...
    std::vector<float> z_interpolate_left = interpolatefi(p.depth, lm.depth, dy);
    std::vector<float> z_interpolate_right = interpolatefi(p.depth, rm.depth, dy);
    ...
    for (size_t i = 0; i < dy; i++)
    {
        ...
        CanvasPoint left(p_to_leftx[i], increment, z_interpolate_left[i]);
        left.texturePoint = TexturePoint(texture_p_to_leftx[i], p.texturePoint.y + roundl(i*texture_left_ratio));
        CanvasPoint right(p_to_rightx[i], increment, z_interpolate_right[i]);
...
std::vector<CanvasPoint> fill_triangle(CanvasTriangle tri){
    ...
    CanvasPoint lm,rm;
    if((top.x == bottom.x && top.x == tri.v1().x)){return line(top,bottom);}
    if((top.y == bottom.y && top.y == tri.v1().y)){return line(top,bottom);}
